...
git commit in app/ Hook fires → background AST rebuild of parent graph
git commit in web/ Same (existing hook redirected to parent)
Manual code change Per CLAUDE.md, run graphify update .. from app/ or web/
Doc/image change Run /graphify --update (needs LLM for semantic re-extract)
...
- QuestionCall logo + short tagline (e.g. _"Get expert answers in 15 minutes"_)
